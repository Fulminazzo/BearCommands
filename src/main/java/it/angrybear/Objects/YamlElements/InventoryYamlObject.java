...
public class InventoryYamlObject extends YamlObject<Inventory> {
    ...
    @Override
    public Inventory load(ConfigurationSection configurationSection, String path) throws Exception {
        ConfigurationSection inventorySection = configurationSection.getConfigurationSection(path);
        if (inventorySection == null) return null;

        String title = inventorySection.getString("title");
        int size = inventorySection.getInt("size");

        YamlObject<ItemStack[]> yamlContents = YamlObject.newObject(ItemStack[].class, yamlPairs);
    ...
    public void dump(ConfigurationSection configurationSection, String path) throws Exception {
        ...
        ConfigurationSection inventorySection = configurationSection.createSection(path);
