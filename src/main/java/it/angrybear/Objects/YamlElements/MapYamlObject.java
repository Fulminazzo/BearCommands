...
public class MapYamlObject<K, V> extends IterableYamlObject<Map<K, V>, V> {
    ...
    @Override
    public Map<K, V> load(ConfigurationSection configurationSection, String path) throws Exception {
        ConfigurationSection mapSection = configurationSection.getConfigurationSection(path);
        if (mapSection == null) return null;
        super.load(configurationSection, path);
        this.object = new HashMap<>();
        for (String key : mapSection.getKeys(false)) {
            if (key.equals("value-class")) continue;
            YamlObject<V> yamlObject = YamlObject.newObject(vClass, yamlPairs);
    ...
    public void dump(ConfigurationSection fileConfiguration, String path) throws Exception {
        ...
        ConfigurationSection mapSection = fileConfiguration.createSection(path);
        if (object.isEmpty()) return;
        vClass = (Class<V>) object.values().stream().findAny().orElse(null).getClass();
        for (K key : object.keySet()) {
            YamlObject<V> yamlObject = YamlObject.newObject(object.get(key), yamlPairs);
